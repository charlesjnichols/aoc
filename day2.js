...
const part1_compute = create_compute(12, 2);
_.range(0, _.countBy(input)[','], 4).some(part1_compute.compute);
...
nouns.forEach(noun => {
    verbs.forEach(verb => {
        const part2_compute = create_compute(noun, verb);
        _.range(0, _.countBy(input)[','], 4).some(part2_compute.compute);
