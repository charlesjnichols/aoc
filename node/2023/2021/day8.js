...
const inputz = _(
  ...
  .groupBy((l) => l.length)
  .value();
// console.log(input['2'].length + input['3'].length + input['4'].length + input['7'].length);
...
cgf aefbdgc gf bagf gedacf ecgfb cdfeb acgbe ebcfga cedbga | gf fbgcade cbegfda acbdfeg`
    ...
    .map((l) => {
      const s = l.split("|");
      return {
        pattern: s[0].trim().split(" "),
        display: s[1].trim().split(" "),
      };
    });
...
// const eight = input.pattern.filter(l => l.length==2);
const answer = l_input.map((input) => {
  const one_pattern = _(input.pattern)
    .filter((l) => l.length === 2)
    .map((l) => l.split(""))
    .value()[0];
  const seven_pattern = _(input.pattern)
    .filter((l) => l.length === 3)
    .map((l) => l.split(""))
    .value()[0];
  const four_pattern = _(input.pattern)
    .filter((l) => l.length === 4)
    .map((l) => l.split(""))
    .value()[0];
  const eight_pattern = _(input.pattern)
    .filter((l) => l.length === 7)
    .map((l) => l.split(""))
    .value()[0];

  const aaaa = _.difference(seven_pattern, one_pattern)[0];
  const bbbb_possible = _(four_pattern)
    .difference(one_pattern, seven_pattern)
    .value();
  const ffff = _(input.pattern)
    .map((l) => l.split(""))
    .flatMap()
    .countBy()
    .findKey((l) => l === 9);
  const cccc = one_pattern.filter((l) => l != ffff)[0];

  const two_pattern = _(input.pattern)
    .filter((l) => l.indexOf(ffff) === -1)
    .map((l) => l.split(""))
    .value()[0];

  const five_count = _(input.pattern)
    .filter((l) => l.length === 5)
    .map((l) => l.split(""))
    .value();
  const three_pattern = five_count
    .map((l) => (_.intersection(l, one_pattern).length === 2 ? l : null))
    .filter((l) => l != null)[0];
  const five_pattern = _(input.pattern)
    .filter((l) => l.length === 5)
    .filter((l) => l != two_pattern.join("") && l != three_pattern.join(""))
    .value()[0]
    .split("");

  const sixes = _(input.pattern)
    .filter((l) => l.length === 6)
    .value();

  const nine_pattern = _(input.pattern)
    .filter((l) => l.length === 6)
    .map((l) => l.split(""))
    .filter((l) => _.intersection(l, three_pattern).length === 5)
    .value()[0];
  sixes.splice(sixes.indexOf(nine_pattern.join("")), 1);
  const six_pattern = _(sixes)
    .map((l) => l.split(""))
    .filter((l) => _.intersection(l, five_pattern).length === 5)
    .value()[0];
  sixes.splice(sixes.indexOf(six_pattern.join("")), 1);
  const zero_pattern = sixes[0].split("");

  // console.log(zero_pattern);
  // console.log(one_pattern);
  // console.log(two_pattern);
  // console.log(three_pattern);
  // console.log(four_pattern);
  // console.log(five_pattern);
  // console.log(six_pattern);
  // console.log(seven_pattern);
  // console.log(eight_pattern);
  // console.log(nine_pattern);

  const key = [
    zero_pattern.sort().join(""),
    one_pattern.sort().join(""),
    two_pattern.sort().join(""),
    three_pattern.sort().join(""),
    four_pattern.sort().join(""),
    five_pattern.sort().join(""),
    six_pattern.sort().join(""),
    seven_pattern.sort().join(""),
    eight_pattern.sort().join(""),
    nine_pattern.sort().join(""),
  ];
  // console.log(key);
  // console.log(input.display);

  return input.display
    .map((l) => key.indexOf(l.split("").sort().join("")))
    .join("");
});

console.log(_.sum(answer.map(l => Number.parseInt(l))));
