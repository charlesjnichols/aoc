...
const path = (wire) => {
    ...
    wire.forEach(x => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            visited.push(at.x + ',' + at.y);
        }
    });
    return visited;
}

const visited_one = path(wire_one);
const visited_two = path(wire_two);

const both = _.intersection(visited_one, visited_two);

const part1 = _(both)
                .map(x => _.sum(x.split(',').map(x => Math.abs(Number.parseInt(x)))))
                .min();
console.log(part1);
// 3247

const part2 = _(both)
                .map(x => visited_one.indexOf(x)+visited_two.indexOf(x)+2)
                .min();
console.log(part2);
//48054